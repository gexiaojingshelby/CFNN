...
class NBADataset_detect(data.Dataset):
    """
    Volleyball Dataset for PyTorch
    """
    def __init__(self, tracks,frames, anns, image_path, args, is_training=True):
        super(NBADataset_detect, self).__init__()
        self.tracks=tracks
        self.frames = frames
        self.anns = anns
        self.image_path = image_path
    ...
    def load_samples(self, frames):
        ...
        OH, OW=self.image_size
        images, activities, boxes = [], [],[]
        ...
        for i, (vid, sid, fid) in enumerate(frames):
            ...
            temp_boxes=[]
            try:
                for i,track in enumerate(self.tracks[(vid,fid_o)][fid_o]):
                    if(i>=self.num_boxes): 
                        break
                    y1,x1,y2,x2,_ = track
