...
import models.res_patch as models
...
# Module Introduction
parser.add_argument('--intro', default='', type=str, help='volleyball or nba')
...
parser.add_argument('--top_k1', default=12, type=int, help='transformer channel dimension')
parser.add_argument('--top_k2', default=6, type=int, help='transformer channel dimension')
...
parser.add_argument('--neighbor_size', default=1, type=int, help='correlation neighborhood size')


# Transformer parameters
parser.add_argument('--depth', default=8, type=int, help='depth of blocks')
parser.add_argument('--num_heads', default=2, type=int, help='depth of blocks')
parser.add_argument('--dropout', default=0.1, type=int, help='')
...
parser.add_argument('--hidden_dim', default=512, type=int, help='transformer channel dimension')

# Training parameters
parser.add_argument('--random_seed', default=3, type=int, help='random seed for reproduction')
...
parser.add_argument('--draw_freq', default=10, type=int, help='print frequency')
parser.add_argument('--draw_dir', default='', type=str, help='print frequency')
...
def main():
    ...
    exp_name = '[%s]_DFGAR_<%s>_(%s)' % (args.dataset, time_str,args.intro)
    ...
    # input_tensor = torch.randn(args.batch, args.num_frame, 3, 720, 1280).to('cuda')   # 3 channels, 224x224 images
    # macs, params = get_model_complexity_info(model, (args.num_frame, 3, 720, 1280),verbose=True)
    # print_log(save_path,'{:<30} {:<8}'.format('Computation conplexity:',macs))
    # print_log(save_path,'{:<30} {:<8}'.format('Parameters:',params))
...
def validate(test_loader, model, criterion, epoch):
    ...
    for i, (images, activities) in enumerate(test_loader):
        ...
        # compute output
        # score,sorted_index= model(images)
        #draw_attention(epoch,i,images,sorted_index)
        score= model(images)
